(function() {
  var charsetRegExp, contentDisposition, cookie, deprecate, escapeHtml, extname, http, isAbsolute, merge, mime, normalizeType, normalizeTypes, onFinished, path, res, resolve, send, sendfile, setCharset, sign, vary,
  ...
  sendfile = function(res, file, options, callback) {
    var done, onaborted, ondirectory, onend, onerror, onfile, onfinish, onstream;
    done = false;
    onaborted = function() {
      var err;
      if (done) {
    ...
    if (options.headers) {
      file.on('headers', function(res) {
        var i, key, keys, len1, obj, results;
        obj = options.headers;
        keys = Object.keys(obj);
        results = [];
        for (i = 0, len1 = keys.length; i < len1; i++) {
          key = keys[i];
          results.push(res.setHeader(key, obj[key]));
        }
        return results;
      });
    }
  ...
  res.format = function(obj) {
    ...
    if (key) {
      ...
      err.types = normalizeTypes(key).map(function(o) {
        return o.value;
      });
    }
    return this;
  ...
  res.attachment = function(filename) {
    ...
    this.set('Content-Dispositon', contentDisposition(filename));
    return this;
  };
  ...
  res.set = res.header = function(field, val) {
    ...
    if (arguments.length === 2) {
      ...
      for (i = 0, len1 = field.length; i < len1; i++) {
        key = field[i];
