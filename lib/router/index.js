(function() {
  ...
  appendMethods = function(list, addition) {
    var j, len, method;
    for (j = 0, len = addtion.length; j < len; j++) {
      method = addtion[j];
  ...
  mergeParams = function(params, parent) {
    var _idx, i_param, i_parent, int_in_params, int_in_parent, j, max_param, max_parent, obj, ref;
    ...
    for (_idx = j = ref = max_param; ref <= 0 ? j <= 0 : j >= 0; _idx = ref <= 0 ? ++j : --j) {
  ...
  restore = function(fn, obj) {
    var idx, j, len, prop, props, vals;
    props = new Array(arguments.length - 2);
    vals = new Array(arguments.length - 2);
    for (idx = j = 0, len = props.length; j < len; idx = ++j) {
      prop = props[idx];
    ...
    return function(err) {
      var index, k, len1;
      for (index = k = 0, len1 = props.length; k < len1; index = ++k) {
        prop = props[index];
  ...
  proto.process_params = function(layer, called, req, res, done) {
    var i, keys, param, paramCallback, paramIndex, params;
    params = this.params;
    keys = layer.keys;
    ...
    param = function(err) {
      var key, name, paramCallbacks, paramCalled, paramVal;
      if (err) {
        return done(err);
      }
      if (i >= keys.length) {
